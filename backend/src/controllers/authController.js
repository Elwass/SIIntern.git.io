...
export async function login(req, res) {
  try {
    ...
    if (!EMAIL_REGEX.test(normalizedEmail) || !password) {
      return res.status(400).json({ success: false, message: 'Email dan password wajib valid.' })
    }

    const [rows] = await pool.query('SELECT * FROM users WHERE email = ? LIMIT 1', [normalizedEmail])
    console.log('DB query result (login user found):', rows.length)
    const user = rows[0]
    if (!user) return res.status(404).json({ success: false, message: 'User tidak ditemukan.' })

    const isMatch = await bcrypt.compare(password, user.password_hash)
    if (!isMatch) return res.status(401).json({ success: false, message: 'Password salah.' })
    if (user.status !== 'active') return res.status(403).json({ success: false, message: 'Akun belum aktif. Verifikasi email dulu.' })
    ...
    await pool.query(
      `INSERT INTO email_otps (user_id, email, purpose, otp_hash, expires_at, attempts, created_at)
       VALUES (?, ?, 'signin', ?, DATE_ADD(NOW(), INTERVAL ? MINUTE), 0, NOW())`,
      [user.id, user.email, otpHash, OTP_EXPIRY_MINUTES],
    )

    await sendOtpEmail(user.email, otp, 'signin')
    return res.json({ success: true, message: 'OTP login sent to email' })
  } catch (err) {
...
export async function verifyLogin(req, res) {
  try {
    const { userId, otp } = req.body
    if (!userId || !/^\d{6}$/.test(String(otp || ''))) {
      return res.status(400).json({ success: false, message: 'userId/otp tidak valid.' })
    }

    const [users] = await pool.query('SELECT * FROM users WHERE id = ? LIMIT 1', [Number(userId)])
    const user = users[0]
    if (!user) return res.status(404).json({ success: false, message: 'User tidak ditemukan.' })

    const [rows] = await pool.query(
      `SELECT * FROM email_otps WHERE user_id = ? AND purpose = 'signin' AND used_at IS NULL ORDER BY id DESC LIMIT 1`,
      [Number(userId)],
    )
    const otpRow = rows[0]
    if (!otpRow) return res.status(400).json({ success: false, message: 'OTP tidak ditemukan.' })
    if (new Date(otpRow.expires_at) < new Date()) return res.status(400).json({ success: false, message: 'OTP expired.' })
    if (otpRow.attempts >= MAX_OTP_ATTEMPTS) return res.status(429).json({ success: false, message: 'OTP attempts exceeded.' })

    const ok = await bcrypt.compare(String(otp), otpRow.otp_hash)
    if (!ok) {
      await pool.query('UPDATE email_otps SET attempts = attempts + 1 WHERE id = ?', [otpRow.id])
      return res.status(400).json({ success: false, message: 'OTP invalid.' })
    }
    ...
    return res.json({ success: true, message: 'Login success', data: { accessToken, user: buildPublicUser(user) } })
  } catch (err) {
    console.error('Verify login error detail:', err)
    return res.status(500).json({ success: false, message: err.message, stack: err.stack })
  }
}
