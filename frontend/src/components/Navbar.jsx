...
function DropdownItem({ item }) {
  return (
    <div className="relative group h-full flex items-center">
      <button className="relative h-full px-3 text-sm font-bold text-gray-800 transition-colors duration-200 hover:text-red-700 flex items-center gap-1 after:absolute after:left-0 after:bottom-0 after:h-[2px] after:w-0 after:bg-red-600 after:transition-all after:duration-200 group-hover:after:w-full">
        {item.label}
        ...
        <div className="absolute left-0 top-full opacity-0 -translate-y-1 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-150">
          <div className="min-w-52 bg-white rounded-none shadow-md p-2 space-y-1">
            {item.children.map((child) => (
              <button key={child} className="relative w-full text-left px-3 py-2 text-sm text-gray-800 transition-colors duration-200 hover:text-red-700 after:absolute after:left-3 after:bottom-0 after:h-[2px] after:w-0 after:bg-red-600 after:transition-all after:duration-200 hover:after:w-[calc(100%-1.5rem)]">
                {child}
              </button>
            ))}
          </div>
    ...
    <header className={`fixed top-0 left-0 z-50 w-full bg-white border-b border-red-600 transition-transform duration-300 ease-in-out ${isVisible ? 'translate-y-0' : '-translate-y-full'} ${isScrolledState ? 'shadow-sm' : ''}`}>
      <div className="w-full px-10 xl:px-16 py-4 min-h-[88px] md:min-h-[96px] flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center h-full gap-1">
          {menuItems.map((item) =>
            item.to ? (
              <NavLink key={item.label} to={item.to} className={({ isActive }) => `group relative h-full px-3 text-sm font-bold flex items-center transition-colors duration-200 after:absolute after:left-0 after:bottom-0 after:h-[2px] after:bg-red-600 after:w-0 after:transition-all after:duration-200 ${isActive ? 'text-red-600 font-semibold after:w-full' : 'text-gray-800 hover:text-red-700 group-hover:after:w-full'}`}>
                {() => (
                  <>
                    {item.label}
                  </>
